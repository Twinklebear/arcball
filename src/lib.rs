...
impl<F: BaseFloat> ArcballCamera<F> {
    ...
    pub fn get_mat4(&self) -> Matrix4<F> { self.camera }
    /// Get the inverse view matrix
    pub fn get_inv_camera(&self) -> Matrix4<F> { self.inv_camera }
    ...
    pub fn pan(&mut self, mouse_delta: Vector2<F>) {
        ...
        let delta = Vector4::new(mouse_delta.x * self.inv_screen[0],
		                 -mouse_delta.y * self.inv_screen[1],
                                 F::zero(), F::zero()) * zoom_dist;
        let motion = self.inv_camera * delta;
        self.center_translation = Matrix4::from_translation(Vector3::new(motion.x, motion.y, motion.z))
